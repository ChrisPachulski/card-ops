...
- **MSR achievability** (use `docs/bonus-strategy.md`):
  ...
  - Tier 2 (acceleration needed): if < 80%, suggest safe acceleration strategies from `docs/bonus-strategy.md`
  - Tier 3: do NOT recommend manufactured spending. If MSR is unreachable organically, factor that into the Bonus Value score.
- **Bonus value calculation** (use `docs/points-valuations.md`):
...
- **Referral check**: If user holds a card in the same family, note referral bonus opportunity and value (see `docs/bonus-strategy.md#Referral Bonus Overlay`)
...
- **Clawback warning**: Note the issuer's clawback rules (see `docs/bonus-strategy.md#Clawback Risk Framework`). Minimum: "Keep card open 12+ months."
...
- **Statement credit programs**: Check if card participates in Amex Offers, Chase Offers, or issuer credit stack (see `docs/scoring-benchmarks.md#Statement Credit Programs`). Estimate realistic annual value based on user's redemption profile (50-70% active, 20-40% casual). Include in Perks Fit scoring.
...
Using `docs/application-timing.md`:
...
- **Inquiry sensitivity**: Where does this issuer rank? (see `docs/application-timing.md`). If applying to a sensitive issuer, check total recent inquiries.
...
- If the card replaces an existing fee card: remind user to call for retention offer before closing/downgrading (see `docs/bonus-strategy.md#Retention Offer Strategy`)
- If a product change path exists for the replaced card: note it in the report (see `docs/bonus-strategy.md#Upgrade/Downgrade Paths`)
